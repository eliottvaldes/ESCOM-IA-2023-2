...
% nombre = F A B I A N  V A L D E S
nombre = "[5 0 1 8 0 13 27 22 0 11 3 4 19]";
nombre = str2num(nombre);
nombre = uint8(nombre);

longitud_nom = length(nombre);
...
while usuario == 0
    ...
    if(cuadro == 1)
     ...
     if (x6<0)
        ...
        mensaje = sprintf('Base: %s | númeroBin: %s, Numero:%d',cadena,num_neg_bin,x6);
        disp(mensaje);
        ...
        if((x5(f1,c1) == 0))
           ...
           elseif((x5(f1,c1) == -1 || x5(f1,c1) == 1) && run == 14) %1
            ...
            if (x7<0)
               ...
               num_neg_bin = dec2bin(num_neg);
               cadena = [cadena num_neg_bin];
               mensaje = sprintf('Base: %s , \tnúmeroBin: %s \tNumero:%d',base,num_neg_bin,x7);
            else
                num_bin = dec2bin(x6, bits);
                cadena = [cadena '0'];
                cadena = [cadena num_bin];
                mensaje = sprintf('Base: %s , \tnúmeroBin: %s, \tNumero:%d',base,num_bin,x7);
            end
            disp(mensaje);
    ...
    mensaje = sprintf('Base Final: 1010');
    disp(mensaje);
    cadena = [cadena '1010'];
    disp('El valor del cuaddro decodificado es:');
    disp(cadena);
    disp('La matriz original es:');
    disp(x1);
    disp('La matriz procesada es:');
    disp(x5);
    usuario = 1;
end
disp("la longitud de mi nombre es:")
disp(longitud_nom)
