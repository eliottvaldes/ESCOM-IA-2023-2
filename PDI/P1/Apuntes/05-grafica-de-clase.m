...
% graficando los puntos extraidos de la iamgen
figure(1)
% class one
plot(class_one(1,:),class_one(2,:),class_one(3,:), 'ro', 'MarkerEdgeColor', 'r', 'MarkerSize', 10)
grid on % display grid -> use to plot more points in the same figure
hold on % hold the figure -> use to plot more points in the same figure

% class two
plot(class_two(1,:),class_two(2,:),class_two(3,:), 'go', 'MarkerEdgeColor', 'g', 'MarkerSize', 10)
grid on % display grid -> use to plot more points in the same figure
hold on % hold the figure -> use to plot more points in the same figure

% class three
plot(class_three(1,:),class_three(2,:),class_three(3,:), 'bo', 'MarkerEdgeColor', 'b', 'MarkerSize', 10)

% unknown class
plot(unknown_class(1,:),unknown_class(2,:),unknown_class(3,:), 'k*', 'MarkerEdgeColor', 'k', 'MarkerSize', 10)

% define a legend
legend('Pimiento verde', 'Cebollin', 'Pimiento rojo', 'Desconocido')
